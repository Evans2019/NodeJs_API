module.exports = function (app, bcrypt, connection , transporter,jwt,uniqid,mime, fs) {
    ...
    //Delete wordAssociation deleteWordAssociation
                        ...
                        connection.query(sql, [values], (err, rows) => {
                            if (err) {
                                ...
                                let HelperOptions = {
                                    ...
                                    text: 'Click on this link to verify your email http://localhost:8080/api/verification?token='+accessToken+''
                                };
    ...
    app.post('/api/course', (req, res) => {
        ...
        connection.query(sqlCheck, [CouserName,language], (err, results, fields) => {
           if(results.length > 0 ){
               ...
               //Insert statement to database
               var sql = "INSERT INTO vambo.courses ( coursename,Language, Datecreated ) VALUES ?";
               var values =[[CouserName,language,datetime]];
               connection.query(sql, [values], (err, rows) => {
        ...
        connection.query(sqlCheck, [req.body.coursename,req.body.language,LessonName], (err, row2, fields) => {
            if (err) {
                ...
                if (row2.length > 0 ) {
                    ...
                    var sql = "INSERT INTO vambo.lessons (lessonId, lessonname,Language,course, Datecreated ) VALUES ?";
                    var values =[[LessonID,LessonName,req.body.language,req.body.coursename, datetime]];
                    connection.query(sql, [values], (err, rows) => {
                        if (err ) {
                            console.log(err)
                        } else {
